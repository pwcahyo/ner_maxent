...
class Func:
	...
	def convert_terbilang_to_number(self, sentence):
		...
		for arr_index, arr_data in enumerate(terbilang) :
			#print terbilang[index]
			temp_terbilang = terbilang[arr_index]

			#for index, data in enumerate(angka):
			# pecah data terbilang
			angka = temp_terbilang.split(" ")
			...
			while index < len(angka):
				...
				if data["label"] :
					kalimat = kalimat.replace(temp_terbilang+"/num", angka_str+"/num")
				else:
					kalimat = kalimat.replace(temp_terbilang, angka_str)
...
#main process
#data = Func()
#print data.convert_terbilang_to_number("lima puluh lima buah")
